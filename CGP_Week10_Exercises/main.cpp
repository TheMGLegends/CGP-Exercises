...
int main(int argc, char* argv[])
{
	...
	//GameObject sonic = factory.CreateGameObject(pngTexture, false);
	...
	//gameObjectList.push_back(sonic);
	...
	while (keepRunning)
	{
		...
		while (SDL_PollEvent(&sdlEvent))
		{
			...
			*/
			...
			if (!state[SDL_SCANCODE_W] && !state[SDL_SCANCODE_S] && !state[SDL_SCANCODE_A] && !state[SDL_SCANCODE_D]) {
				sonic.rb.m_velocityX = 0;
				sonic.rb.m_velocityY = 0;
			}

			if (state[SDL_SCANCODE_W])
			{
				//std::cout << "W was pressed!" << std::endl;
				sonic.rb.m_velocityY -= sonic.rb.m_speed;
				//sonic.boxCollider.m_y -= movementSpeed * deltaTime;
				//sonic.circleCollider.m_y -= movementSpeed * deltaTime;
			...
			{
				sonic.rb.m_velocityX -= sonic.rb.m_speed;
				//sonic.boxCollider.m_x -= movementSpeed * deltaTime;
				//sonic.circleCollider.m_x -= movementSpeed * deltaTime;
			...
			if (state[SDL_SCANCODE_Z]) {
				...
				sonic.m_texture = pngTexture;
				Time::currentTimeScale = 1.0f;
			}

			if (state[SDL_SCANCODE_X]) {
				factory.DestroyGameObject(sonic);
			}
		...
		float magnitude = sqrtf((sonic.rb.m_velocityX * sonic.rb.m_velocityX) + (sonic.rb.m_velocityY * sonic.rb.m_velocityY));

		if (magnitude > 1.0f)
		{

			sonic.rb.m_velocityX /= magnitude;
			sonic.rb.m_velocityY /= magnitude;
		}

		std::cout << magnitude << std::endl;

		sonic.boxCollider.m_x += sonic.rb.m_velocityX * sonic.rb.m_speed * deltaTime;
		sonic.boxCollider.m_y += sonic.rb.m_velocityY * sonic.rb.m_speed * deltaTime;
